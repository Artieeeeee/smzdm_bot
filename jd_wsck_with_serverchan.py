...
def send_notification(title, content,summary):
    ...
    api_key = os.environ.get("WP_APP_TOKEN_ONE", "")
    uids= os.environ.get("WP_APP_MAIN_UID", "")
...
def getcookie_wskey(key):
    ...
    for num in range(0,5):
        ...
        if token!="xxx":
            ...
            printf(f"【警告】{unquote(pin)}在获取token失败，等待5秒后重试")
            time.sleep(5)
            randomuserAgent()
            if os.environ.get("DY_PROXY") is not None:
                proxys = get_proxy_api(proxy_url)
...
def main():
    ...
    try:
        if os.environ.get("WP_APP_TOKEN_ONE")==None or os.environ.get("WP_APP_MAIN_UID")==None:
            ...
            if os.environ.get("WP_APP_TOKEN_ONE")!="" and os.environ.get("WP_APP_MAIN_UID")!="":
                printf('检测到已配置Wxpusher相关变量,将使用Wxpusher发送通知')
                iswxpusher=True
    except:
        iswxpusher=False

    if not iswxpusher and not isserverchan:
        printf('没有配置Wxpusher或Server酱,将尝试调用系统默认sendNotify.py发送通知')
    ...
    if resurt2!="":
        ...
        # 发送通知逻辑整合
        if isserverchan:
            send_serverchan("JD_WSCK转换结果", resurt)
        if iswxpusher:
            send_notification("JD_WSCK转换结果", resurt, summary)
        
        if not isserverchan and not iswxpusher:
            if hadsend:
